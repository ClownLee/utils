...
exports.arrayMin = exports.arrayMax = exports.findParentArr = exports.findParentTree = exports.deepClone = exports.treeToArray = exports.arrayToTree = void 0;
...
var arrayToTree = function (items, options) {
    ...
    if (!options.idField)
        options.idField = 'id';
    if (!options.pidField)
        options.pidField = 'pid';
    if (!options.childrenField)
        options.childrenField = 'children';
    var idField = options.idField, pidField = options.pidField, childrenField = options.childrenField;
    var findFun = function (parent, child) {
        return parent[idField] === child[pidField];
    };
    var tree = {};
    var _loop_1 = function (item) {
        var ptr = items.find(function (parent) { return findFun(parent, item); });
        if (ptr === undefined)
            ptr = tree;
...
var treeToArrayVisitNode = function (node, items, options, parentId) {
    ...
    if (node[childrenField]) {
        try {
            for (var _b = __values(node[childrenField]), _c = _b.next(); !_c.done; _c = _b.next()) {
                var child = _c.value;
...
var getParent = function (arr, id, items, options) {
    var idField = options.idField, pidField = options.pidField;
    var tmp = arr.filter(function (i) { return i[idField] === id; })[0];
    if (tmp && tmp[pidField]) {
        items.unshift(tmp);
...
var findParentTree = function (tree, sonId, options) {
    ...
    var items = [];
    var tmp = arr.filter(function (i) { return i[idField] === sonId; })[0];
    if (tmp) {
        items.unshift(tmp);
        getParent(arr, tmp[pidField], items, options);
    }
