...
var SM4 = /** @class */ (function () {
   ...
   */
  function SM4(config) {
    var keyBuffer = crypt_1.default.stringToArrayBufferInUtf8(config.key);
    ...
    if (config.iv !== undefined && config.iv !== null) {
      // need iv
      ivBuffer = crypt_1.default.stringToArrayBufferInUtf8(config.iv);
   ...
   * @memberof SM4
   */
  SM4.prototype.doBlockCrypt = function (blockData, roundKeys) {
   ...
   * @memberof SM4
   */
  SM4.prototype.spawnEncryptRoundKeys = function () {
    // extract mk in key
   ...
   * @memberof SM4
   */
  SM4.prototype.rotateLeft = function (x, y) {
    return (x << y) | (x >>> (32 - y));
  };
  /**
   ...
   * @memberof SM4
   */
  SM4.prototype.linearTransform1 = function (b) {
    return (
      b ^
      this.rotateLeft(b, 2) ^
   ...
   * @memberof SM4
   */
  SM4.prototype.linearTransform2 = function (b) {
   ...
   * @memberof SM4
   */
  SM4.prototype.tauTransform = function (a) {
    return (
      (Sbox[(a >>> 24) & 0xff] << 24) |
      (Sbox[(a >>> 16) & 0xff] << 16) |
      (Sbox[(a >>> 8) & 0xff] << 8) |
   ...
   * @memberof SM4
   */
  SM4.prototype.tTransform1 = function (z) {
   ...
   * @memberof SM4
   */
  SM4.prototype.tTransform2 = function (z) {
    var b = this.tauTransform(z);
   ...
   * @memberof SM4
   */
  SM4.prototype.padding = function (originalBuffer) {
   ...
   * @memberof SM4
   */
  SM4.prototype.dePadding = function (paddedBuffer) {
    if (paddedBuffer === null) {
      return null;
    }
    var paddingLength = paddedBuffer[paddedBuffer.length - 1];
    var originalBuffer = paddedBuffer.slice(
      0,
   ...
   * @memberof SM4
   */
  SM4.prototype.uint8ToUint32Block = function (uint8Array, baseIndex) {
    if (baseIndex === void 0) {
      baseIndex = 0;
    }
   ...
   */
  SM4.prototype.encrypt = function (plaintext) {
    var plainByteArray = crypt_1.default.stringToArrayBufferInUtf8(plaintext);
    var padded = this.padding(plainByteArray);
    var blockTimes = (padded && padded.length) || 0 / UINT8_BLOCK;
    var outArray = new Uint8Array((padded && padded.length) || 0);
    if (this.mode === 'cbc') {
      // CBC mode
      ...
      for (var i = 0; i < blockTimes; i++) {
        ...
        for (var l = 0; l < UINT8_BLOCK; l++) {
          outArray[roundIndex + l] =
            (cipherBlock[parseInt(''.concat(l / 4))] >> (((3 - l) % 4) * 8)) &
            0xff;
        }
      }
    } else {
    ...
    if (this.cipherType === 'base64') {
      return crypt_1.default.arrayBufferToBase64(outArray);
    } else {
      // text
      return crypt_1.default.utf8ArrayBufferToString(outArray);
    }
  };
  /**
   ...
   * @memberof SM4
   */
  SM4.prototype.decrypt = function (ciphertext) {
    // get cipher byte array
    var cipherByteArray = new Uint8Array();
    if (this.cipherType === 'base64') {
      // cipher is base64 string
      cipherByteArray = crypt_1.default.base64ToArrayBuffer(ciphertext);
    } else {
      // cipher is text
      cipherByteArray = crypt_1.default.stringToArrayBufferInUtf8(ciphertext);
    }
    ...
    if (this.mode === 'cbc') {
      ...
      for (var i = 0; i < blockTimes; i++) {
        ...
        var plainBlockBeforeXor = this.doBlockCrypt(
          block,
        ...
        for (var l = 0; l < UINT8_BLOCK; l++) {
          outArray[roundIndex + l] =
            (plainBlock[parseInt(''.concat(l / 4))] >> (((3 - l) % 4) * 8)) &
            0xff;
        }
      }
    } else {
    ...
    return crypt_1.default.utf8ArrayBufferToString(depaddedPlaintext);
  };
  return SM4;
})();
exports.default = SM4;
