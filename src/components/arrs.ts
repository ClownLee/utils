 ...
 */
 export const arrayToTree = (items: any[], options: OBJ = {}): OBJ | any[] => {
   ...
   */
  const treeToArrayVisitNode = (node: OBJ, items: OBJ, options: OBJ, parentId: number | string | undefined): void => {
    const { idField, pidField, childrenField } = options;
...
export const deepClone = (obj: OBJ | any[]): OBJ | any[]  => {
   ...
   * @param {*} options 配置数据
   */
  const getParent = (arr: any[], id: number | string, items: any[], options: OBJ): void => {
   ...
   * @returns 
   */
export const findParentTree = (tree: any[], sonId: number | string, options: OBJ = {}): any[] => {
    if (!options.idField) options.idField = 'id';
    if (!options.pidField) options.pidField = 'pid';
    if (!options.childrenField) options.childrenField = 'children';
    const { idField, pidField } = options;
   ...
   * @returns 
   */
export const findParentArr = (arr: any[], sonId: number | string, options: OBJ = {}): any[] => {
    if (!options.idField) options.idField = 'id';
    if (!options.pidField) options.pidField = 'pid';
    const { idField, pidField } = options;
